$(document).ready(function(){
    ...
    $("#btnGuardarBeneficiario").click(function(event){
        ...
        var NoCuenta,NombreAlumno,Edad,Correo,Telefono,Usuario,Contraseña,ClaveAdmin,ClaveISEMYN,accion;
    ...
    $("#btnGuardarPago").click(function(event){
        // Declaración de variables a usar:
        var clave1, cantidad, mes;
        // Se recoge los datos de los campos de texto:
        clavePago = $("#txtClavePago").val();
        cantidad = $("#txtCantidadPago").val();
        mes = $("#txtMesPago").val();
        // Verificamos que acción texto tiene el botón:
        ...
        // Envíamos los datos al servidor (backend), es un archivo php:
            $.ajax({
                url: "./php/servidor.php",
                type: "GET",
                data: {accion1: accion1, clavePago: clavePago,
                cantidad: cantidad, mes: mes},
                success: function(respuestaPHP){
                    // Se verifica la respuesta del servidor:
                    if (respuestaPHP == "1"){
    ...
    $("#btnGuardarRetiro").click(function(event){
        // Declaración de variables a usar:
        var clave2, motivo,cantidad,fecha,accion2;
        // Se recoge los datos de los campos de texto:
        claveRetiro = $("#txtClaveRetiro").val();
        motivo = $("#txtMotivoRetiro").val();
        cantidad = $("#txtCantidadRetiro").val();
        fecha = $("#txtFechaRetiro").val();
        // Verificamos que acción texto tiene el botón:
        ...
        // Envíamos los datos al servidor (backend), es un archivo php:
            $.ajax({
                url: "./php/servidor.php",
                type: "GET",
                data: {accion2: accion2, claveRetiro: claveRetiro,
                motivo: motivo, cantidad: cantidad, fecha: fecha},
                success: function(respuestaPHP){
                    // Se verifica la respuesta del servidor:
                    if (respuestaPHP == "1"){
...
function editarBeneficiario ( id, NoCuenta, NombreAlumno, Edad, Correo, Telefono, Usuario , Contraseña, ClaveAdmin, ClaveISEMYN){
    $("#txtClaveBeneficiario").val(NoCuenta);
...
function eliminarPago(idPago){
    alertify.confirm("¿Desea eliminar el pago con id "+idPago+"?",function(respuesta){
    if(respuesta)
    {
        ...
        $.ajax({
            url:"./php/servidor.php",
            type:"GET",
            data:{accion1:accion1,idPago:idPago},
                success:function(respuestaPHP){
                    //se verifica la respuesta del servidor
                    if(respuestaPHP=="1"){
...
function eliminarRetiro(idRetiro){
    alertify.confirm("¿Desea eliminar el Retiro con idRetiro "+idRetiro+"?",function(respuesta){
        if(respuesta)
        {
            ...
            $.ajax({
                url:"./php/servidor.php",
                type:"GET",
                data:{accion2:accion2,idRetiro:idRetiro},
                success:function(respuestaPHP){
                    //se verifica la respuesta del servidor
                    if(respuestaPHP=="1"){
                        alertify.success("Retiro eliminado exitosamente");
                        $("#mostrarRetiro").hide();
                        $("#mostrarRetiro").load("./php/mostrarRetiro.php");
